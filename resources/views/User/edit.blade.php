        ...
        <x-cards.card class="userShow__card">
            ...
            <header class="userShow__header">
                @if (auth()->user()->id == $user->id)
                    <div
                        style="display: flex; justify-content: space-between; position: absolute; top: 10px; width: 95%;">
                        <div>
                        ...
                        <div>
                            <form action="{{ route('auth.destroy', auth()->user()->id) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button
                                    style="padding: 1rem; width: 100px; background: #313131; color: white; border: none; outline: none; border-radius: 0.5rem;">
                                    Logout
                                </button>
                            </form>
                        </div>
                ...
                <div class="userShow__header__div">
                    <x-ui.form action="{{ route('user.avatarUpdate', $user) }}" style="position: relative; padding: 1rem;" enctype="multipart/form-data">
                        ...
                        <div style="position: absolute; left: 55%; top: 10px;">  
                            ...
                            <label for="avatar">
                                <img src="/settings.png" style="height: 40px; width: 40px; border-radius: 50%; background-color: #313131; padding: 0.2rem;"/>
                            </label>
                        </div>
                        <img src="{{ $user->avatar }}" class="userShow__header__div__img" style="margin-bottom: 0.4rem; border: 1px solid #313131;"/>
